...
const ButtonSend = ({ sending, sent, ...props }: IButton) => {
	return (
		<button {...props} className={styles.button} disabled={sending}>
			<div
				className={`${styles.state} ${sent ? styles.stateSent : styles.stateDefault} ${sending ? styles.stateSending : ""}`}>
				...
				<p>
					<span style={{ "--i": 0 } as CSSProperties}>S</span>
					<span style={{ "--i": 1 } as CSSProperties}>e</span>
					<span style={{ "--i": 2 } as CSSProperties}>n</span>
					<span style={{ "--i": 3 } as CSSProperties}>{sent ? "t" : "d"}</span>
				</p>
			</div>
		</button>
	);
